...
void                   i2c_using_i3c_demo_thread_entry(void);
...
#define DUMMY_SLAVE2        0x55
    ...
    uint8_t slave_addr[MAX_SLAVE_SUPPORTED] = {BMI_ADDR, DUMMY_SLAVE1, DUMMY_SLAVE2};
        ...
        for (i = 0; i < MAX_SLAVE_SUPPORTED; i++) {
            ...
            while (retry_cnt) {
                ...
                if (cb_event_flag == I3C_CB_EVENT_SUCCESS) {
                    /* TX Success: Got ACK from slave */
                    printf("\r\n \t\t >> i=%" PRId32
                           " TX Success: Got ACK from slave addr:0x%" PRIx8 ".\r\n",
                           i,
                           slave_addr[i]);
                    break;
                }
