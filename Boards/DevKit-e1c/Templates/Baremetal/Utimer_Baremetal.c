 ...
 *           - Configuring the UTIMER Channel 2 for counter start triggering mode.
 *           - Configuring the UTIMER Channel 3 for driver A, double buffering capture mode.
 * @bug      None.
 * @Note     None
 ******************************************************************************/
...
ARM_DRIVER_GPIO *ptrDrv = &ARM_Driver_GPIO_(GPIO0);
...
static int32_t gpio_init(ARM_UTIMER_MODE mode)
{
    ...
    {
        /* init P0_0 as GPIO */
        ret = pinconf_set (PORT_0, PIN_0, PINMUX_ALTERNATE_FUNCTION_0, 0);
        if(ret != ARM_DRIVER_OK) {
            printf("\r\n Error in PINMUX.\r\n");
            return -1;
        }

        ret = ptrDrv->Initialize(GPIO0_PIN0, NULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to initialize GPIO0_PIN0 as GPIO\n");
            return -1;
        }

        ret = ptrDrv->PowerControl(GPIO0_PIN0, ARM_POWER_FULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to Power up GPIO0_PIN0\n");
            return -1;
        }

        ret = ptrDrv->SetDirection(GPIO0_PIN0, GPIO_PIN_DIRECTION_OUTPUT);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set direction for GPIO0_PIN0\n");
            return -1;
        }

        ret = ptrDrv->SetValue(GPIO0_PIN0, GPIO_PIN_OUTPUT_STATE_LOW);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set value for GPIO0_PIN0\n");
            return -1;
        }

        /* init P0_1 as GPIO */
        ret = pinconf_set (PORT_0, PIN_1, PINMUX_ALTERNATE_FUNCTION_0, 0);
        if(ret != ARM_DRIVER_OK) {
            printf("\r\n Error in PINMUX.\r\n");
            return -1;
        }

        ret = ptrDrv->Initialize(GPIO0_PIN1, NULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to initialize GPIO0_PIN1 as GPIO\n");
            return -1;
        }

        ret = ptrDrv->PowerControl(GPIO0_PIN1, ARM_POWER_FULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to Power up GPIO0_PIN1\n");
            return -1;
        }

        ret = ptrDrv->SetDirection(GPIO0_PIN1, GPIO_PIN_DIRECTION_OUTPUT);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set direction for GPIO0_PIN1\n");
            return -1;
        }

        ret = ptrDrv->SetValue(GPIO0_PIN1, GPIO_PIN_OUTPUT_STATE_LOW);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set value for GPIO0_PIN1\n");
            return -1;
        }
    }
    ...
    {
        /* init P0_2 as GPIO */
        ret = pinconf_set (PORT_0, PIN_2, PINMUX_ALTERNATE_FUNCTION_0, 0);
        if(ret != ARM_DRIVER_OK) {
            printf("\r\n Error in PINMUX.\r\n");
            return -1;
        }

        ret = ptrDrv->Initialize(GPIO0_PIN2, NULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to initialize GPIO0_PIN2 as GPIO\n");
            return -1;
        }

        ret = ptrDrv->PowerControl(GPIO0_PIN2, ARM_POWER_FULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to Power up GPIO0_PIN2\n");
            return -1;
        }

        ret = ptrDrv->SetDirection(GPIO0_PIN2, GPIO_PIN_DIRECTION_OUTPUT);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set direction for GPIO0_PIN2\n");
            return -1;
        }

        ret = ptrDrv->SetValue(GPIO0_PIN2, GPIO_PIN_OUTPUT_STATE_LOW);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set value for GPIO0_PIN2\n");
            return -1;
        }

        /* init P0_3 as GPIO */
        ret = pinconf_set (PORT_0, PIN_3, PINMUX_ALTERNATE_FUNCTION_0, 0);
        if(ret != ARM_DRIVER_OK) {
            printf("\r\n Error in PINMUX.\r\n");
            return -1;
        }

        ret = ptrDrv->Initialize(GPIO0_PIN3, NULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to initialize GPIO0_PIN3 as GPIO\n");
            return -1;
        }

        ret = ptrDrv->PowerControl(GPIO0_PIN3, ARM_POWER_FULL);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to Power up GPIO0_PIN3\n");
            return -1;
        }

        ret = ptrDrv->SetDirection(GPIO0_PIN3, GPIO_PIN_DIRECTION_OUTPUT);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set direction for GPIO0_PIN3\n");
            return -1;
        }

        ret = ptrDrv->SetValue(GPIO0_PIN3, GPIO_PIN_OUTPUT_STATE_LOW);
        if (ret != ARM_DRIVER_OK) {
            printf("ERROR: Failed to set value for GPIO0_PIN3\n");
            return -1;
        }
    }
...
static void utimer_basic_mode_app(void)
{
    ...
    count_array[1] = 80000000;    /*< over flow count value >*/
...
static void utimer_buffering_mode_app (void)
{
    ...
    count_array[0] = 0;             /*< Initial counter value>*/
    count_array[1] = 80000000;      /*< Over flow count value for First Iteration>*/
    count_array[2] = 160000000;     /*< Over flow count value for Second Iteration>*/
    count_array[3] = 240000000;     /*< Over flow count value for Third Iteration>*/
...
static void utimer_trigger_mode_app(void)
{
    ...
    /*
     * utimer channel 2 is configured for utimer trigger mode.
     * chan_event_a_rising_b_0 event is used to start the counter.
     **/
    ...
    ret = pinconf_set (PORT_6, PIN_5, PINMUX_ALTERNATE_FUNCTION_4, PADCTRL_READ_ENABLE);
    if(ret != ARM_DRIVER_OK) {
        printf("\r\n Error in PINMUX.\r\n");
    }
    ...
    ret = ptrDrv->SetValue(GPIO0_PIN0, GPIO_PIN_OUTPUT_STATE_HIGH);
    if ((ret != ARM_DRIVER_OK)) {
...
static void utimer_capture_mode_cb_func(uint8_t event)
{
    ...
    ptrDrv->SetValue(GPIO0_PIN2, GPIO_PIN_OUTPUT_STATE_LOW);
}
...
static void utimer_capture_mode_app(void)
{
    ...
    /*
     * utimer channel 3 is configured for utimer input capture mode (selected driver A, double buffer).
     * chan_event_a_rising_b_0 event is used to capture counter value.
     */
     ...
     */
    count_array[0] = 0;            /*< initial counter value >*/
    count_array[1] = 160000000;    /*< over flow count value >*/

    /* capture mode pin config */
    ret = pinconf_set(PORT_6, PIN_6, PINMUX_ALTERNATE_FUNCTION_4, PADCTRL_READ_ENABLE);
    if(ret != ARM_DRIVER_OK) {
        printf("\r\n Error in PINMUX.\r\n");
    }

    ret = pinconf_set(PORT_6, PIN_7, PINMUX_ALTERNATE_FUNCTION_4, PADCTRL_READ_ENABLE);
    if(ret != ARM_DRIVER_OK) {
        printf("\r\n Error in PINMUX.\r\n");
    }
    ...
    /* Config Trigger for to capture counter using chan_event_a_rising_b_0 */
    ret = ptrUTIMER->ConfigTrigger (channel, &trig_config);
    if (ret != ARM_DRIVER_OK) {
        printf("utimer channel %d trigger configuration failed \n", channel);
        goto error_capture_mode_poweroff;
    ...
    for(int index=0; index<3; index++)
    {
        /* Delay of 100 ms */
        sys_busy_loop_us (100000);
        ret = ptrDrv->SetValue(GPIO0_PIN2, GPIO_PIN_OUTPUT_STATE_HIGH);
        if ((ret != ARM_DRIVER_OK)) {
            printf("ERROR: Failed to configure\n");
        }
