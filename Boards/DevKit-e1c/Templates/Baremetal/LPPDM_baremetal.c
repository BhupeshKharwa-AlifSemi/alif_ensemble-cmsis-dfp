...
static int32_t lppdm_pinmux(void)
{
    ...
    status = pinconf_set(PORT_5, PIN_6, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_READ_ENABLE);
    if(status)
        printf("pinconf_set failed\n");

    /* channel 2_3 data line */
    status = pinconf_set(PORT_5, PIN_7, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_READ_ENABLE);
    if (status)
        return ERROR;

    /* channel 4_5 data line */
    status = pinconf_set(PORT_7, PIN_5, PINMUX_ALTERNATE_FUNCTION_2, PADCTRL_READ_ENABLE);
    if (status)
        return ERROR;

    /* channel 6_7 data line */
    status = pinconf_set(PORT_7, PIN_7, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_READ_ENABLE);
    if (status)
        return ERROR;

    /* Channel 0_1 clock line */
    status = pinconf_set(PORT_5, PIN_4, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_DRIVER_DISABLED_HIGH_Z);
    if(status)
        printf("pinconf_set failed\n");

    /* Channel 2_3 clock line */
    status = pinconf_set(PORT_5, PIN_5, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_DRIVER_DISABLED_HIGH_Z);
    if (status)
        return ERROR;

    /* Channel 4_5 clock line */
    status = pinconf_set(PORT_7, PIN_4, PINMUX_ALTERNATE_FUNCTION_2, PADCTRL_DRIVER_DISABLED_HIGH_Z);
    if (status)
        return ERROR;

    /* Channel 6_7 clock line */
    status = pinconf_set(PORT_7, PIN_6, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_DRIVER_DISABLED_HIGH_Z);
    if (status)
        return ERROR;
