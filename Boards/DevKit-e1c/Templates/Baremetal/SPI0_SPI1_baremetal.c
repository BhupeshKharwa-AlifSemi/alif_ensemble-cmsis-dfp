 ...
 * @version  V1.0.0
 * @date     19-July-2024
 * @brief    baremetal demo application for SPI0 and SPI1 on Balletto.
...
static int32_t pinmux_config(void)
{
    ...
    ret = pinconf_set(PORT_5, PIN_0, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_READ_ENABLE);
    if (ret)
    {
    ...
    ret = pinconf_set(PORT_8, PIN_7, PINMUX_ALTERNATE_FUNCTION_3, PADCTRL_READ_ENABLE);
    if (ret)
    {
