...
#define COMPARE_CTRL_DRV_CYCLE_END                                                                 \
    (uint32_t)(COMPARE_CTRL_DRV_CYCLE_END_1 | COMPARE_CTRL_DRV_CYCLE_END_0)
...
#define BUF_OP_CTRL_CAPTURE_A_BUF_OP                                                               \
    ((uint32_t) (BUF_OP_CTRL_CAPTURE_A_BUF_OP_BIT0 | BUF_OP_CTRL_CAPTURE_A_BUF_OP_BIT1))
#define BUF_OP_CTRL_CAPTURE_B_BUF_OP_BIT0 ((uint32_t) 0x00040000)
#define BUF_OP_CTRL_CAPTURE_B_BUF_OP_BIT1 ((uint32_t) 0x00080000)
#define BUF_OP_CTRL_CAPTURE_B_BUF_OP                                                               \
    ((uint32_t) (BUF_OP_CTRL_CAPTURE_B_BUF_OP_BIT0 | BUF_OP_CTRL_CAPTURE_B_BUF_OP_BIT1))
#define BUF_OP_CTRL_CNTR_BUF_OP_BIT0 ((uint32_t) 0x00100000)
#define BUF_OP_CTRL_CNTR_BUF_OP_BIT1 ((uint32_t) 0x00200000)
#define BUF_OP_CTRL_CNTR_BUF_OP                                                                    \
    ((uint32_t) (BUF_OP_CTRL_CNTR_BUF_OP_BIT0 | BUF_OP_CTRL_CNTR_BUF_OP_BIT1))
...
#define BUF_OP_CTRL_COMPARE_A_BUF_EVENT                                                            \
    ((uint32_t) (BUF_OP_CTRL_COMPARE_A_BUF_EVENT_BIT0 | BUF_OP_CTRL_COMPARE_A_BUF_EVENT_BIT1))
...
#define BUF_OP_CTRL_COMPARE_B_BUF_EVENT                                                            \
    ((uint32_t) (BUF_OP_CTRL_COMPARE_B_BUF_EVENT_BIT0 | BUF_OP_CTRL_COMPARE_B_BUF_EVENT_BIT1))
...
#define FAULT_CTRL_FAULT_EN                                                                        \
    ((uint32_t) (FAULT_CTRL_FAULT_EN_A_TRIG0 | FAULT_CTRL_FAULT_EN_A_TRIG1 |                       \
...
typedef struct _utimer_channel_config {
    ...
    bool fault_type;   /**< For fault triggers: SET: low until counter stop, CLEAR: until
                          overflow/underflow event >*/
    bool fixed_buffer; /**< SET: Enable Fixed buffer feature, CLEAR: Disable Fixed buffer feature in
                          compare mode >*/
    bool driver_a_start_state;  /**< initial state of output driver A, SET: Driver state is HIGH,
                                   CLEAR: Driver state is LOW >*/
    bool driver_a_stop_state;   /**< end state of output driver A, SET: Driver state is HIGH, CLEAR:
                                   Driver state is LOW >*/
    bool buffering_type;        /**< Buffering type, SET: Double, CLEAR: Single >*/
    bool driver_b_start_state;  /**< initial state of output driver B, SET: Driver state is HIGH,
                                   CLEAR: Driver state is LOW >*/
    bool driver_b_stop_state;   /**< end state of output driver B, SET: Driver state is HIGH, CLEAR:
                                   Driver state is LOW >*/
    ...
    bool buf_trough_n_crest;    /**< buffer config for triangle timer- SET: buffer at crest&trough,
                                   CLEAR: buffer at trough >*/
    uint8_t capt_buffer_type_A; /**< Only for Capture mode: Buffering type for Drive A Single or
                                   Double >*/
    uint8_t capt_buffer_type_B; /**< Only for Capture mode: Buffering type for Drive B Single or
                                   Double >*/
    uint8_t driver_a_at_comp_match; /**< COMPARE: state of driver A output on count matches with
                                       compare buffer values >*/
    uint8_t driver_a_at_cycle_end;  /**< state of driver A output on end of cycle,
                                       counter_overflow/counter_underflow >*/
    uint8_t driver_b_at_comp_match; /**< COMPARE: state of driver B output on count matches with
                                       compare buffer values >*/
    uint8_t driver_b_at_cycle_end;  /**< state of driver B output on end of cycle,
                                       counter_overflow/counter_underflow >*/
...
/**
  \fn           static inline void utimer_counter_stop (UTIMER_Type *utimer, uint8_t channel, bool
  clear_count) \brief        Stop utimer channel counter with counter clear option \param[in] utimer
  : Pointer to utimer register block \param[in]    channel     : utimer channel number \param[in]
  clear_count : counter clear option \return       none
*/
...
/**
  \fn           static inline void utimer_clear_interrupt (UTIMER_Type *utimer, uint8_t channel,
  uint8_t interrupt) \brief        clear utimer channel interrupt \param[in]    utimer      :
  Pointer to utimer register block \param[in]    channel     : utimer channel number \param[in]
  interrupt   : interrupt needs to be cleared \return       none
*/
...
/**
  \fn           static inline void utimer_unmask_interrupt (UTIMER_Type *utimer, uint8_t channel,
  uint8_t interrupt) \brief        unmask utimer channel interrupt \param[in]    utimer      :
  Pointer to utimer register block \param[in]    channel     : utimer channel number \param[in]
  interrupt   : interrupt needs to be unmasked \return       none
*/
...
/**
  \fn           static inline void utimer_mask_interrupt (UTIMER_Type *utimer, uint8_t channel,
  uint8_t interrupt) \brief        mask utimer channel interrupt \param[in]    utimer      : Pointer
  to utimer register block \param[in]    channel     : utimer channel number \param[in]    interrupt
  : interrupt needs to be masked \return       none
*/
...
/**
  \fn           static inline void utimer_enable_duty_cycle (UTIMER_Type *utimer, uint8_t channel,
  utimer_channel_config *ch_config) \brief        enable utimer channel duty cycle \param[in] utimer
  : Pointer to utimer register block \param[in]    channel     : utimer channel number \param[in]
  ch_config   : pointer for utimer channel configuration structure \return       none
*/
static inline void utimer_enable_duty_cycle(UTIMER_Type *utimer, uint8_t channel,
                                            utimer_channel_config *ch_config)
{
    if (ch_config->driver_A) {
        utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DUTY_CYCLE_CTRL |=
            (DUTY_CYCLE_CTRL_DC_ENABLE_A | DUTY_CYCLE_CTRL_DC_FORCE_A |
             DUTY_CYCLE_CTRL_DC_UNDERFLOW_A | (ch_config->dc_value) << 2);
    }
    if (ch_config->driver_B) {
        utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DUTY_CYCLE_CTRL |=
...
/**
  \fn          void utimer_config_direction (UTIMER_Type *utimer, uint8_t channel, UTIMER_TYPE type,
  utimer_channel_config *ch_config) \brief       configure counter type for the UTIMER instance.
  ...
  \return      none
*/
void utimer_config_direction(UTIMER_Type *utimer, uint8_t channel, UTIMER_COUNTER_DIR dir,
                             utimer_channel_config *ch_config);

/**
  \fn          void utimer_config_mode (UTIMER_Type *utimer, uint8_t channel, UTIMER_MODE mode,
  utimer_channel_config *ch_config) \brief       configure counter mode for the UTIMER instance.
  \param[in]   utimer      Pointer to the UTIMER register map
  \param[in]   channel     channel number
  ...
  \return      none
*/
void utimer_config_mode(UTIMER_Type *utimer, uint8_t channel, UTIMER_MODE mode,
                        utimer_channel_config *ch_config);

/**
  \fn          void utimer_set_count (UTIMER_Type *utimer, uint8_t channel, UTIMER_SET_COUNTER_TYPE
  counter_type, utimer_channel_config *ch_config) \brief       set counter value for the UTIMER
  instance. \param[in]   utimer          Pointer to the UTIMER register map \param[in]   channel
  channel number \param[in]   counter_type    counter type \param[in]   value           counter
  value \return      none
*/
...
/**
  \fn          uint32_t utimer_get_count (UTIMER_Type *utimer, uint8_t channel,
  UTIMER_GET_COUNTER_TYPE counter, utimer_channel_config *ch_config) \brief       get counter
  direction for the UTIMER instance. \param[in]   utimer         Pointer to the UTIMER register map
  \param[in]   channel        channel number
...
/**
  \fn          void utimer_config_trigger (UTIMER_Type *utimer, uint8_t channel, UT_TRIGGER_CONFIG
  *config_control, utimer_channel_config *ch_config) \brief       configure trigger for the UTIMER
  instance. \param[in]   utimer          Pointer to the UTIMER register map \param[in]   channel
  channel number \param[in]   config_control  Pointer to a trigger configure type argument
  \param[in]   ch_config       Pointer to the UTIMER channel specific config structure
  \return      none
*/
void utimer_config_trigger(UTIMER_Type *utimer, uint8_t channel,
                           UTIMER_TRIGGER_CONFIG *config_control, utimer_channel_config *ch_config);
