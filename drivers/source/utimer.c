                             ...
                             utimer_channel_config *ch_config)
{
    ...
    case UTIMER_COUNTER_DOWN:
        {
            utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_CNTR_CTRL |=
                (CNTR_CTRL_EN | CNTR_CTRL_DIR_DOWN);
    ...
    case UTIMER_COUNTER_TRIANGLE:
        {
            if (ch_config->fixed_buffer) {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_CNTR_CTRL |=
                    (CNTR_CTRL_EN | CNTR_CTRL_TRIANGLE_ONE_SHOT);
            } else {
                if (ch_config->buf_trough_n_crest) {
                    utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_CNTR_CTRL |=
                        (CNTR_CTRL_EN | CNTR_CTRL_TRIANGLE_BUF_TROUGH_CREST);
                } else {
                    utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_CNTR_CTRL |=
                        (CNTR_CTRL_EN | CNTR_CTRL_TRIANGLE_BUF_TROUGH);
                }
            }
            break;
        }
    ...
    case UTIMER_MODE_BASIC:
        {
            ...
            if (ch_config->driver_A) {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_COMPARE_CTRL_A |=
                    (COMPARE_CTRL_DRV_DRIVER_EN | (ch_config->driver_a_at_cycle_end << 2) |
                     (ch_config->driver_a_start_state << 4) |
                     (ch_config->driver_a_stop_state << 6));
            }

            if (ch_config->driver_B) {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_COMPARE_CTRL_B |=
                    (COMPARE_CTRL_DRV_DRIVER_EN | (ch_config->driver_b_at_cycle_end << 2) |
    ...
    case UTIMER_MODE_BUFFERING:
        {
            ...
            if (ch_config->buffering_type) {  // SET: Double Buffering
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                    BUF_OP_CTRL_CNTR_BUF_OP_BIT1;
            } else {  // CLEAR: Single Buffering
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                    BUF_OP_CTRL_CNTR_BUF_OP_BIT0;
            }
    ...
    case UTIMER_MODE_CAPTURING:
        {
            ...
            if (ch_config->buffer_operation) {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                    (BUF_OP_CTRL_CAPTURE_BUF_EN | (ch_config->capt_buffer_type_A << 16) |
            ...
            if (ch_config->driver_A) {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_COMPARE_CTRL_A |=
                    (COMPARE_CTRL_DRV_COMPARE_EN | COMPARE_CTRL_DRV_DRIVER_EN |
                     (ch_config->driver_a_at_cycle_end << 2) | (ch_config->driver_a_at_comp_match) |
                     (ch_config->driver_a_start_state << 4) |
                     (ch_config->driver_a_stop_state << 6));

                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                    ((ch_config->comp_buffer_at_crest << 24) |
                     (ch_config->comp_buffer_at_trough << 25));

                if (ch_config->buffer_operation) {
                    utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                        (BUF_OP_CTRL_COMPARE_BUF_EN | (ch_config->buffering_type << 26));
                }

                if (ch_config->fixed_buffer) {
                    utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                        BUF_OP_CTRL_FORCE_COMPARE_BUF_OP;
                }
                if (ch_config->dma_clear_en) {
                    utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_COMPARE_CTRL_A |=
                        COMPARE_CTRL_DRV_DMA_CLEAR_EN;
                }
            }

            if (ch_config->driver_B) {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_COMPARE_CTRL_B |=
                    (COMPARE_CTRL_DRV_DRIVER_EN | COMPARE_CTRL_DRV_COMPARE_EN |
                     (ch_config->driver_b_at_cycle_end << 2) | (ch_config->driver_b_at_comp_match) |
                     (ch_config->driver_b_start_state << 4) |
                     (ch_config->driver_b_stop_state << 6));

                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                    ((ch_config->comp_buffer_at_crest << 28) |
                     (ch_config->comp_buffer_at_trough << 29));

                if (ch_config->buffer_operation) {
                    utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                        (BUF_OP_CTRL_COMPARE_BUF_EN | (ch_config->buffering_type << 30));
                }
                if (ch_config->fixed_buffer) {
                    utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_BUF_OP_CTRL |=
                        BUF_OP_CTRL_FORCE_COMPARE_BUF_OP;
...
/**
  \fn          void utimer_set_count (UTIMER_Type *utimer, uint8_t channel, UTIMER_COUNTER
  counter_type, uint32_t value) \brief       set counter value for the UTIMER instance. \param[in]
  utimer       Pointer to the UTIMER register map \param[in]   channel      channel number
...
/**
  \fn          uint32_t utimer_get_count (UTIMER_Type *utimer, uint8_t channel, UTIMER_COUNTER
  counter, utimer_channel_config *ch_config) \brief       get counter direction for the UTIMER
  instance. \param[in]   utimer         Pointer to the UTIMER register map \param[in]   channel
  channel number \param[in]   counter_type   counter type \return      current counter value
*/
...
/**
  \fn          void utimer_config_trigger (UTIMER_Type *utimer, uint8_t channel,
  UTIMER_TRIGGER_CONFIG *config_control, utimer_channel_config *ch_config) \brief       configure
  trigger for the UTIMER instance. \param[in]   utimer          Pointer to the UTIMER register map
  \param[in]   channel         channel number
                           ...
                           UTIMER_TRIGGER_CONFIG *config_control, utimer_channel_config *ch_config)
{
    if (config_control->src_type == UTIMER_SRC_0) {
        ...
        case UTIMER_TRIGGER_START:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_START_1_SRC &=
                    ~(CNTR_SRC1_PGM_EN); /* disabling pgm_en */
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_START_0_SRC |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_STOP:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_STOP_1_SRC &=
                    ~(CNTR_SRC1_PGM_EN); /* disabling pgm_en */
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_STOP_0_SRC |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_CLEAR:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_CLEAR_1_SRC &=
                    ~(CNTR_SRC1_PGM_EN); /* disabling pgm_en */
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_CLEAR_0_SRC |=
                    config_control->trigger_type;
                break;
            }
        ...
        case UTIMER_TRIGGER_DOWNCOUNT:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DOWN_0_SRC |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_CAPTURE_A:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_TRIG_CAPTURE_SRC_A_0 |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_CAPTURE_B:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_TRIG_CAPTURE_SRC_B_0 |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_DMA_CLEAR_A:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DMA_CLEAR_SRC_A_0 |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_DMA_CLEAR_B:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DMA_CLEAR_SRC_B_0 |=
                    config_control->trigger_type;
                break;
            }
        }
    ...
    if (config_control->src_type == UTIMER_SRC_1) {
        ...
        case UTIMER_TRIGGER_DOWNCOUNT:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DOWN_1_SRC |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_CAPTURE_A:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_TRIG_CAPTURE_SRC_A_1 |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_CAPTURE_B:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_TRIG_CAPTURE_SRC_B_1 |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_DMA_CLEAR_A:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DMA_CLEAR_SRC_A_1 |=
                    config_control->trigger_type;
                break;
            }
        case UTIMER_TRIGGER_DMA_CLEAR_B:
            {
                utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_DMA_CLEAR_SRC_B_1 |=
                    config_control->trigger_type;
                break;
            }
        }
    ...
    if (config_control->src_type == UTIMER_FAULT_TRIGGER) {
        if (ch_config->driver_A) {
            utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_FAULT_CTRL |=
                (config_control->trigger_type | ch_config->fault_type << 8);
        }
        if (ch_config->driver_B) {
            utimer->UTIMER_CHANNEL_CFG[channel].UTIMER_FAULT_CTRL |=
                (config_control->trigger_type << 16 | ch_config->fault_type << 24);
        }
    }
