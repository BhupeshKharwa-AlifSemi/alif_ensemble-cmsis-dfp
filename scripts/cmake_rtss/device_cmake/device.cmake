...
set (DEVICE_SKU_CONFIG          "${DEVICE_PATH}/soc/AE722F80F55D5/config")
set (DEVICE_SKU_INC             "${DEVICE_PATH}/soc/AE722F80F55D5/include")
...
if (PROC STREQUAL M55_HP)
    ...
    set (DEVICE_SOC_INC           "${DEVICE_SKU_INC}/hp")
else ()
    set (SUBSYSTEM_SRC_DIR        "${DEVICE_PATH}/core/rtss_he/e7/AE722F80F55D5/source")
    set (DEVICE_SOC_INC           "${DEVICE_SKU_INC}/he")
endif ()
...
file (GLOB DEVICE_SRC   "${DEVICE_COMMON_SRC}/*.c" "${DEVICE_COMMON_SRC_DIR}/*.c" "${DEVICE_CORE_SRC_DIR}/*.c"
                        "${SUBSYSTEM_SRC_DIR}/*.c" "${SOC_SRC_DIR}/*.c" "${DEVICE_SYSTEM_SRC}/*.c")
include_directories (${DEVICE_ROOT})
include_directories (${DEVICE_COMMON_INC})
include_directories (${DEVICE_CORE_CONFIG_DIR})
include_directories (${DEVICE_SYSTEM_INC})
include_directories ("${DEVICE_PATH}/soc/e7/AE722F80F55D5/include")
include_directories ("${DEVICE_SKU_INC}")
include_directories ("${DEVICE_SKU_CONFIG}")
include_directories ("${DEVICE_SOC_INC}")
...
if (PROC STREQUAL M55_HP)
    ...
    set (SCRIPT_DIR      "${DEVICE_PATH}/core/rtss_hp/linker")
else ()
    ...
    set (SCRIPT_DIR      "${DEVICE_PATH}/core/rtss_he/linker")
endif ()
...
if (COMPILER STREQUAL ARMCLANG)
    if (BOOT STREQUAL TCM)
        set (SCRIPT_NAME            "linker_ac6_tcm")
    elseif (BOOT STREQUAL MRAM)
        set (SCRIPT_NAME            "linker_ac6_mram")
    else ()
        message(FATAL_ERROR  "${Red}INVALID BOOT ... !!!${ColourReset}")
    endif()
...
elseif ( (COMPILER STREQUAL GCC) OR (COMPILER STREQUAL CLANG) )
    if (BOOT STREQUAL TCM)
        set (SCRIPT_NAME            "linker_gnu_tcm")
    elseif (BOOT STREQUAL MRAM)
        set (SCRIPT_NAME            "linker_gnu_mram")
    else ()
        message(FATAL_ERROR  "${Red}INVALID BOOT ... !!!${ColourReset}")
    endif()

    #string(TOLOWER      ${COMPILER}     COMP)
    set (LINKER_SCRIPT_FILE             ${SCRIPT_DIR}/${SCRIPT_NAME}.ld.src)
    set (CMAKE_LINKER_SCRIPT            ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${SCRIPT_NAME}.ld)
