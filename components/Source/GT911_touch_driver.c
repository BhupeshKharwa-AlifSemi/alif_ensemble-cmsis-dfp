...
/*Driver version*/
static const ARM_DRIVER_VERSION DriverVersion                 = {ARM_TOUCH_SCREEN_API_VERSION,
...
/**
  ...
  */
static int32_t TOUCH_Write(uint8_t dev_addr, uint16_t reg_addr, uint8_t *reg_data,
                           uint8_t num_bytes)
{
    ...
    do {
        ret = I2C_Driver->MasterTransmit(dev_addr, tx_data, num_bytes + GT911_REG_INDX_SIZE, STOP);
...
{
    uint32_t arg = ARM_GPIO_IRQ_POLARITY_LOW | ARM_GPIO_IRQ_EDGE_SENSITIVE_SINGLE |
...
static int32_t ARM_TOUCH_SCREEN_GetState(ARM_TOUCH_STATE *state)
{
    ...
    if (touch_event_gpio) {
        ...
        state->numtouches = ((data & GT911_NUM_TOUCH_POINTS_MSK) > RTE_ACTIVE_TOUCH_POINTS
                                 ? RTE_ACTIVE_TOUCH_POINTS
                                 : (data & GT911_NUM_TOUCH_POINTS_MSK));
