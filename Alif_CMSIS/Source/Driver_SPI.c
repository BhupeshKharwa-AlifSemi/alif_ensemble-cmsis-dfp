...
/**
  \fn          int32_t SPI_DMA_PowerControl(DMA_PERIPHERAL_CONFIG *dma_periph,
...
/**
  ...
  \return      \ref        execution_status
*/
static inline int32_t SPI_DMA_Usermcode(DMA_PERIPHERAL_CONFIG *dma_periph, uint32_t dma_mcode)
{
    int32_t         status;
    ARM_DRIVER_DMA *dma_drv = dma_periph->dma_drv;
...
{
    if (!((SPI->mw_config.cfs >= SPI_MW_CONTROL_FRAME_SIZE_MIN) &&
          (SPI->mw_config.cfs <= SPI_MW_CONTROL_FRAME_SIZE_MAX)))
    {
...
static int32_t ARM_SPI_Initialize(SPI_RESOURCES *SPI, ARM_SPI_SignalEvent_t cb_event)
{
    ...
    if ((SPI->tx_fifo_threshold > SPI_TX_FIFO_DEPTH) ||
        (SPI->tx_fifo_start_level > SPI_TX_FIFO_DEPTH))
    {
        return ARM_DRIVER_ERROR_PARAMETER;
    }
...
static int32_t ARM_SPI_Receive(SPI_RESOURCES *SPI, void *data, uint32_t num)
{
    ...
    if (SPI->dma_enable) {
        ...
        if (SPI->rx_fifo_threshold > 0) {
            SPI->rx_fifo_threshold =
                spi_dma_calc_rx_level(SPI->transfer.rx_total_cnt, SPI->rx_fifo_threshold);
            spi_set_dma_rx_level(SPI->regs, SPI->rx_fifo_threshold);
        }
...
/**
 * @fn      int32_t ARM_SPI_Transfer(SPI_RESOURCES *SPI, const void *data_out, void *data_in,
 * uint32_t num).
 ...
 * @retval  \ref execution_status
 */
static int32_t ARM_SPI_Transfer(SPI_RESOURCES *SPI, const void *data_out, void *data_in,
                                uint32_t num)
{
    if (SPI->state.powered == 0) {
        return ARM_DRIVER_ERROR;
    ...
    if (SPI->mw_enable) {
        if (SPI->transfer.is_master &&
            (SPI->mw_config.transfer_mode == SPI_MW_TRANSFER_MODE_SEQUANTIAL)) {
            /* In master sequential mode receive, tx_total_cnt should be 1 as only one control word
             * is sent */
        ...
        if ((!SPI->transfer.is_master) &&
            (SPI->mw_config.transfer_mode == SPI_MW_TRANSFER_MODE_SEQUANTIAL))
        {
            /* In slave sequential mode transmit, rx_total_cnt should be 1 as only one control word
             * is received */
    ...
    /* If the Frame size is more than 16, check if it is aligned to 4 bytes */
    if ((SPI->transfer.frame_size > 16) &&
        ((((uint32_t) data_in & 0x3U) != 0U) && (((uint32_t) data_out & 0x3U) != 0U)))
    {
        return ARM_DRIVER_ERROR_PARAMETER;
    }
    /* If the Frame size is more than 8 and less than 16, check if it is aligned to 2 bytes */
    if ((SPI->transfer.frame_size > 8) &&
        ((((uint32_t) data_in & 0x1U) != 0U) && (((uint32_t) data_out & 0x1U) != 0U)))
    {
        return ARM_DRIVER_ERROR_PARAMETER;
    }
...
#if SPI_USE_MASTER_SS_SW
                if (arg == 1) {
                    ret = SPI->sw_config.drvGPIO->SetValue(SPI->sw_config.ss_pin,
                                                           SPI->sw_config.active_polarity);
                    if (ret != ARM_DRIVER_OK) {
                        return ret;
                    }
                } else if (arg == 0) {
        ...
        case ARM_SPI_SS_MASTER_SW:
            {
                ...
                ret = SPI->sw_config.drvGPIO->SetDirection(SPI->sw_config.ss_pin,
...
#if (SOC_FEAT_LPSPI_HAS_MASTER_SLAVE && !RTE_LPSPI_CONFIG_MASTER_SLAVE)
