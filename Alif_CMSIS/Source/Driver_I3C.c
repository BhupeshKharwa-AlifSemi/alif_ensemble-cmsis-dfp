...
/**
  ...
  \return      \ref           execution_status
*/
__STATIC_INLINE int32_t I3C_DMA_Start(DMA_PERIPHERAL_CONFIG *dma_periph, ARM_DMA_PARAMS *dma_params)
{
    int32_t         status;
    ARM_DRIVER_DMA *dma_drv = dma_periph->dma_drv;
...
/**
  ...
  \return       \ref execution_status
*/
static int I3Cx_GetSlaveDynamicAddr(I3C_RESOURCES *i3c, uint8_t static_addr, uint8_t *dynamic_addr)
{
    if (!dynamic_addr) {
...
/**
  ...
  \return       \ref execution_status
*/
static int I3Cx_GetSlvsInfo(I3C_RESOURCES *i3c, void *data, const uint8_t value)
{
    uint8_t iter;
    ...
    if (i3c->state.is_master) {
        ...
        /* Performs the following if it's a secondary master */
...
/**
  ...
  \return       \ref execution_status
*/
static int I3Cx_GetSlaveList(I3C_RESOURCES *i3c, uint8_t *addr_list, uint8_t *count)
{
    uint8_t pos = 0U;
...
/**
  ...
  \return       None
*/
static void I3C_DetachSlaves(I3C_RESOURCES *i3c, uint8_t slv_cnt, uint32_t ccc_cmd)
{
    int8_t pos = 0;
...
static ARM_I3C_STATUS I3Cx_GetStatus(I3C_RESOURCES *i3c)
{
    ...
    i3c->status.ibi_slv_addr = i3c->xfer.addr_len;
...
static int I3Cx_MasterSendCommand(I3C_RESOURCES *i3c, ARM_I3C_CMD *ccc)
{
    ...
    else /* command write */
    {
        if (ccc->cmd_id == I3C_CCC_ENTHDR(0U)) {
            ...
            i3c->status.busy         = 0;
        } else if ((ccc->cmd_id > I3C_CCC_ENTHDR(0U)) && (ccc->cmd_id <= I3C_CCC_ENTHDR(7U))) {
...
/**
  ...
  \return       \ref execution_status
*/
static int I3Cx_MasterTransmit(I3C_RESOURCES *i3c, uint8_t addr, const uint8_t *data, uint16_t len)
{
    int32_t index;
    ...
    if ((!i3c->state.enabled) || (!i3c->state.is_master)) {
        return ARM_DRIVER_ERROR;
    }
...
/**
  ...
  \return       \ref execution_status
*/
static int I3Cx_MasterReceive(I3C_RESOURCES *i3c, uint8_t addr, uint8_t *data, uint16_t len)
{
    int32_t index;
...
/**
  ...
  \return       \ref execution_status
*/
static int I3Cx_SlaveTransmit(I3C_RESOURCES *i3c, const uint8_t *data, uint16_t len)
{
#if I3C_DMA_ENABLE
    int32_t ret;
#endif
...
/**
  ...
  \return       \ref execution_status
*/
static int I3Cx_MasterAssignDA(I3C_RESOURCES *i3c, ARM_I3C_CMD *addr_cmd)
{
    int32_t pos      = 0U;
    ...
    if (addr_cmd->cmd_id == I3C_CCC_ENTDAA) {
        ...
        for (iter = 0U; iter < addr_cmd->len; iter++) {
            ...
            if (pos < 0) {
                ...
                i3c_add_slv_to_dat(i3c->regs, pos, addr_cmd->addr, 0);
            }
...
static int32_t I3Cx_Control(I3C_RESOURCES *i3c, uint32_t control, uint32_t arg)
{
    ...
    case I3C_MASTER_SET_SLAVE_NACK_RETRY_COUNT:
        ...
        retry_cnt = ((arg & I3C_SLAVE_NACK_RETRY_COUNT_Msk) >> I3C_SLAVE_NACK_RETRY_COUNT_Pos);
...
#if RTE_I3C_BLOCKING_MODE_ENABLE
        if (i3c->blocking_mode) {
            i3c_master_bus_reset_blocking(i3c->regs, i3c->core_clk, scl_timeout_cnt, loc_var);
        } else
#endif
        {
            i3c_master_bus_reset(i3c->regs, i3c->core_clk, scl_timeout_cnt, loc_var);
        }
        break;
...
/**
  ...
  \return       \ref execution_status
*/
static int32_t I3Cx_Initialize(I3C_RESOURCES *i3c, ARM_I3C_SignalEvent_t cb_event)
{
...
/**
  ...
  \return       \ref execution_status
*/
static int32_t I3Cx_PowerControl(I3C_RESOURCES *i3c, ARM_POWER_STATE state)
{
    bool blocking_mode = false;
#if RTE_I3C_BLOCKING_MODE_ENABLE
    if (i3c->blocking_mode) {
...
/**
  ...
  \return      none
*/
static void I3Cx_DMACallback(uint32_t event, int8_t peri_num, I3C_RESOURCES *i3c)
{
    if (!i3c->cb_event) {
        return;
    }
...
/**
  ...
  \                                            i3c_xfer_t *xfer,
  \                                            uint32_t *event)
  \brief        Handles success case of interrupts
  \param[in]    i3c      : Pointer to i3c resources structure
  \param[in]    xfer     : Pointer to i3c transfer structure
  \param[in]    event    : Event type
  \return       None
*/
static void I3Cx_HandleSuccess(I3C_RESOURCES *i3c, i3c_xfer_t *xfer, uint32_t *event)
{
...
/**
  \fn           static void I3Cx_HandleError(I3C_RESOURCES *i3c,
  \                                         i3c_xfer_t *xfer,
  \                                         uint32_t *event)
  \brief        Handles error case of interrupts
  \param[in]    i3c      : Pointer to i3c resources structure
  \param[in]    xfer     : Pointer to i3c transfer structure
  \param[in]    event    : Event type
  \return       None
*/
static void I3Cx_HandleError(I3C_RESOURCES *i3c, i3c_xfer_t *xfer, uint32_t *event)
{
#if I3C_DMA_ENABLE
    if (i3c->dma_enable) {
...
/**
  \fn           static void I3Cx_IRQHandler(I3C_RESOURCES  *i3c)
  \brief        Handles interrupt
  \param[in]    i3c      : Pointer to i3c resources structure
  \return       None
*/
static void I3Cx_IRQHandler(I3C_RESOURCES *i3c)
{
    ...
    if (xfer->status & I3C_XFER_STATUS_ERROR) {
        I3Cx_HandleError(i3c, xfer, &event);
    } else if (xfer->status & I3C_XFER_STATUS_DONE) { /* check status: Transfer Success? */
        I3Cx_HandleSuccess(i3c, xfer, &event);
    }
...
static int32_t I3C_GetSlaveList(uint8_t *addr_list, uint8_t *count)
{
    return I3Cx_GetSlaveList(&i3c, addr_list, count);
}

static int32_t I3C_GetSlaveDynamicAddr(const uint8_t static_addr, uint8_t *dynamic_addr)
{
...
static int32_t LPI3C_GetSlaveList(uint8_t *addr_list, uint8_t *count)
{
    return I3Cx_GetSlaveList(&LPI3C_RES, addr_list, count);
}

static int32_t LPI3C_GetSlaveDynamicAddr(const uint8_t static_addr, uint8_t *dynamic_addr)
{
