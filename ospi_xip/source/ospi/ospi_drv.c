...
/**
  \fn        void ospi_setup_read(ospi_flash_cfg_t *ospi_cfg, uint32_t addr_len, uint32_t read_len,
  uint32_t wait_cycles) \brief     Set up for Flash read operation \param[in] ospi_cfg : OSPI
  configuration structure \param[in] addr_len : Address length \param[in] read_len : No. of bytes to
  be read \param[in] wait_cycles : Cycles required to read the data \return    none
*/
void ospi_setup_read(ospi_flash_cfg_t *ospi_cfg, uint32_t addr_len, uint32_t read_len,
                     uint32_t wait_cycles)
{
    ...
    val = CTRLR0_IS_MST | (OCTAL << CTRLR0_SPI_FRF_OFFSET) | (TMOD_RO << CTRLR0_TMOD_OFFSET) |
    ...
    if (ospi_cfg->ddr_en) {
        val = TRANS_TYPE_FRF_DEFINED | ((ospi_cfg->ddr_en) << CTRLR0_SPI_DDR_EN_OFFSET) |
              (CTRLR0_INST_L_8bit << CTRLR0_INST_L_OFFSET) | (addr_len << CTRLR0_ADDR_L_OFFSET) |
              (wait_cycles << CTRLR0_WAIT_CYCLES_OFFSET);
    } else {
        val = TRANS_TYPE_STANDARD | ((ospi_cfg->ddr_en) << CTRLR0_SPI_DDR_EN_OFFSET) |
              (CTRLR0_INST_L_8bit << CTRLR0_INST_L_OFFSET) | (addr_len << CTRLR0_ADDR_L_OFFSET) |
              (wait_cycles << CTRLR0_WAIT_CYCLES_OFFSET);
    }
...
void ospi_setup_write_sdr(ospi_flash_cfg_t *ospi_cfg, uint32_t addr_len)
{
    ...
    val = CTRLR0_IS_MST | (SINGLE << CTRLR0_SPI_FRF_OFFSET) | (TMOD_TO << CTRLR0_TMOD_OFFSET) |
          (CTRLR0_DFS_8bit << CTRLR0_DFS_OFFSET);

    ospi_writel(ospi_cfg, ctrlr0, val);
    ospi_writel(ospi_cfg, ctrlr1, 0);

    val = TRANS_TYPE_FRF_DEFINED | (CTRLR0_INST_L_8bit << CTRLR0_INST_L_OFFSET) |
          (addr_len << CTRLR0_ADDR_L_OFFSET) | (0 << CTRLR0_WAIT_CYCLES_OFFSET);
...
void ospi_setup_write(ospi_flash_cfg_t *ospi_cfg, uint32_t addr_len)
{
    ...
    val = CTRLR0_IS_MST | (OCTAL << CTRLR0_SPI_FRF_OFFSET) | (TMOD_TO << CTRLR0_TMOD_OFFSET) |
          (CTRLR0_DFS_8bit << CTRLR0_DFS_OFFSET);
    ...
    if (ospi_cfg->ddr_en) {
        ...
        val = TRANS_TYPE_STANDARD | (CTRLR0_INST_L_8bit << CTRLR0_INST_L_OFFSET) |
              (addr_len << CTRLR0_ADDR_L_OFFSET) | (0 << CTRLR0_WAIT_CYCLES_OFFSET);
    }
...
/**
  ...
  \brief   Send the last byte of data/command after sending address and remaining bytes of command/
  data using ospi_push() \param[in] ospi_cfg : OSPI configuration structure \param[in] data : Last
  byte of data \return  none
*/
...
void ospi_xip_enter(ospi_flash_cfg_t *ospi_cfg, uint16_t incr_command, uint16_t wrap_command)
{
    ...
    val = CTRLR0_IS_MST | (OCTAL << CTRLR0_SPI_FRF_OFFSET) | (0 << CTRLR0_SCPOL_OFFSET) |
          (0 << CTRLR0_SCPH_OFFSET) | (0 << CTRLR0_SSTE_OFFSET) | (TMOD_RO << CTRLR0_TMOD_OFFSET) |
          (CTRLR0_DFS_16bit << CTRLR0_DFS_OFFSET);

    ospi_writel(ospi_cfg, ctrlr0, val);

    val = (OCTAL << XIP_CTRL_FRF_OFFSET) | (0x2 << XIP_CTRL_TRANS_TYPE_OFFSET) |
          (ADDR_L32bit << XIP_CTRL_ADDR_L_OFFSET) | (INST_L8bit << XIP_CTRL_INST_L_OFFSET) |
          (0x0 << XIP_CTRL_MD_BITS_EN_OFFSET) |
          (ospi_cfg->wait_cycles << XIP_CTRL_WAIT_CYCLES_OFFSET) | (0x1 << XIP_CTRL_DFC_HC_OFFSET) |
          (0x1 << XIP_CTRL_DDR_EN_OFFSET) | (0x0 << XIP_CTRL_INST_DDR_EN_OFFSET) |
          (0x1 << XIP_CTRL_RXDS_EN_OFFSET) | (0x1 << XIP_CTRL_INST_EN_OFFSET) |
          (0x0 << XIP_CTRL_CONT_XFER_EN_OFFSET) | (0x0 << XIP_CTRL_HYPERBUS_EN_OFFSET) |
          (0x0 << XIP_CTRL_RXDS_SIG_EN) | (0x0 << XIP_CTRL_XIP_MBL_OFFSET) |
          (0x0 << XIP_PREFETCH_EN_OFFSET) | (0x0 << XIP_CTRL_RXDS_VL_EN_OFFSET);
...
/**
  \fn        void ospi_flash_exit_non_volatile_xip(ospi_flash_cfg_t *ospi_cfg, uint16_t
  incr_command, uint16_t wrap_command) \brief     If the Flash boots in XIP mode, this function will
  be called to exit the non volatile XIP \param[in] ospi_cfg : OSPI configuration structure
...
void ospi_xip_exit(ospi_flash_cfg_t *ospi_cfg, uint16_t incr_command, uint16_t wrap_command)
{
    ...
    val = TRANS_TYPE_FRF_DEFINED | ((ospi_cfg->ddr_en) << CTRLR0_SPI_DDR_EN_OFFSET) |
          (2 << CTRLR0_XIP_MBL_OFFSET) | (1 << CTRLR0_XIP_DFS_HC_OFFSET) |
          (1 << CTRLR0_XIP_INST_EN_OFFSET) | (CTRLR0_INST_L_8bit << CTRLR0_INST_L_OFFSET) |
